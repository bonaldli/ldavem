...
double alhood(double a, double ss, int num_samples, int num_dim) { 
  return(num_samples * (lgamma(num_dim * a) - num_dim * lgamma(a)) + (a - 1) * ss); 
}

double d_alhood(double a, double ss, int num_samples, int num_dim) { 
  return(num_samples * (num_dim * Rf_digamma(num_dim * a) - num_dim * Rf_digamma(a)) + ss); 
}

double d2_alhood(double a, int num_samples, int num_dim) { 
  return(num_samples * (num_dim * num_dim * Rf_trigamma(num_dim * a) - num_dim * Rf_trigamma(a))); 
}
